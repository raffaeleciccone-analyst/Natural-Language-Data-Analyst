"""
Interfaccia Streamlit dell'applicazione.

Qui vive solo il FLUSSO della pagina: cosa si mostra e in quale ordine. I numeri
li calcola `nlda.loader`, il turno di conversazione lo orchestra
`nlda.service.AnalysisService`, la presentazione sta in `nlda.ui_components`.

Tutto è dentro funzioni e l'esecuzione parte da `main()` sotto la guardia
`__main__`: importare questo modulo non disegna nulla, quindi i suoi pezzi sono
raggiungibili da un test. Restano di proposito a livello di modulo le funzioni
decorate con `@st.cache_data`: definirle dentro `main()` le ricreerebbe a ogni
rerun di Streamlit, la chiave di cache cambierebbe ogni volta e la cache non
verrebbe mai colpita.
"""
import html
import io
import os
